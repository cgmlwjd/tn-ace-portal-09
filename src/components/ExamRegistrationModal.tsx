...
export default function ExamRegistrationModal({
  ...
  const handleSchoolSystemChange = (system: string, checked: boolean) => {
    setFormData(prev => {
      let newSelectedSystems = checked ? [...prev.selectedSchoolSystems, system] : prev.selectedSchoolSystems.filter(s => s !== system);

      // 학제를 해제하면 관련된 조합들도 제거
      let newCombinations = checked ? prev.selectedCombinations : prev.selectedCombinations.filter(combo => combo.schoolSystem !== system);
      return {
        ...prev,
  ...
  const handleGradeSelection = (schoolSystem: string, grade: string) => {
    setFormData(prev => {
      const existingIndex = prev.selectedCombinations.findIndex(combo => combo.schoolSystem === schoolSystem && combo.grade === grade);
      let newCombinations;
      if (existingIndex >= 0) {
        // 이미 존재하면 제거
        ...
        // 존재하지 않으면 추가
        newCombinations = [...prev.selectedCombinations, {
          schoolSystem,
          grade
        }];
      }
  ...
  const removeCombination = (schoolSystem: string, grade: string) => {
    setFormData(prev => ({
      ...prev,
      selectedCombinations: prev.selectedCombinations.filter(combo => !(combo.schoolSystem === schoolSystem && combo.grade === grade))
    }));
  };
        ...
        <div className="space-y-6">
          {currentStep === 1 && <div className="space-y-6">
                ...
                <CardContent className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input id="examTitle" placeholder="예: 영어 중간고사 - 2학년" value={formData.title} onChange={e => handleInputChange('title', e.target.value)} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="examDate">시험 일시 *</Label>
                      <Input id="examDate" type="datetime-local" value={formData.examDate} onChange={e => handleInputChange('examDate', e.target.value)} />
                    </div>
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="description">시험 설명</Label>
                    <Textarea id="description" placeholder="시험에 대한 추가 설명을 입력하세요" rows={3} value={formData.description} onChange={e => handleInputChange('description', e.target.value)} />
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {Object.entries(schoolSystemLabels).map(([key, label]) => <div key={key} className="flex items-center space-x-3 p-3 border border-border rounded-lg hover:bg-muted/50 transition-colors">
                          <Checkbox id={`school-${key}`} checked={formData.selectedSchoolSystems.includes(key)} onCheckedChange={checked => handleSchoolSystemChange(key, checked as boolean)} />
                ...
                return <div key={schoolSystem} className="space-y-4 p-4 border border-border rounded-lg bg-muted/20">
                        <Label className="text-base font-medium">{systemLabel} 학년 선택</Label>
                        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                          {grades.map(grade => {
                      const isSelected = formData.selectedCombinations.some(combo => combo.schoolSystem === schoolSystem && combo.grade === grade);
                      return <Button key={grade} variant={isSelected ? "default" : "outline"} size="sm" className="h-8 text-xs" onClick={() => handleGradeSelection(schoolSystem, grade)}>
                                {grade}
                              </Button>;
                    })}
                        </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="min-h-[2rem] flex flex-wrap gap-2">
                      {formData.selectedCombinations.length === 0 ? <p className="text-sm text-muted-foreground">선택된 조합이 없습니다.</p> : formData.selectedCombinations.map((combo, index) => {
                    const systemLabel = schoolSystemLabels[combo.schoolSystem as keyof typeof schoolSystemLabels];
                    return <Badge key={index} variant="secondary" className="flex items-center gap-2 px-3 py-1">
                              <span>{systemLabel}-{combo.grade}</span>
                              <Button variant="ghost" size="sm" className="h-4 w-4 p-0 hover:bg-destructive hover:text-destructive-foreground" onClick={() => removeCombination(combo.schoolSystem, combo.grade)}>
                                <X className="h-3 w-3" />
                              </Button>
                            </Badge>;
                  })}
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  color
                        ...
                        <Checkbox id={key} checked={formData.categories.includes(key as ExamCategory)} onCheckedChange={checked => handleCategoryChange(key as ExamCategory, checked as boolean)} />
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    <div className="min-h-[2rem] flex flex-wrap gap-2">
                      {formData.categories.length === 0 ? <p className="text-sm text-muted-foreground">선택된 카테고리가 없습니다.</p> : formData.categories.map(category => {
                    const {
                      name,
                      icon: Icon,
                      color
                    } = categoryLabels[category];
                    return <Badge key={category} variant="secondary" className="flex items-center gap-1">
          ...
          {currentStep === 2 && <div className="space-y-6">
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  return <div key={category} className="border border-border rounded-lg p-4">
                          ...
                          <div className="space-y-4">
                            ...
                            {/* Math 카테고리 이미지 업로드 */}
                            {(category === 'mcq' || category === 'short' || category === 'math-essay') && <div className="space-y-2">
          ...
          <div className="flex items-center justify-between pt-6 border-t border-border">
            <div className="flex space-x-2">
              ...
              {currentStep === 2 && <Button variant="outline" onClick={handlePrevious}>
            ...
            <div className="flex space-x-2">
              {currentStep === 1 && <Button onClick={handleNext} disabled={!isStep1Valid}>
                  다음
                  <ChevronRight className="h-4 w-4 ml-2" />
                </Button>}
              
              {currentStep === 2 && <Button onClick={handleComplete}>
                  등록 완료
                </Button>}
            </div>
          </div>
